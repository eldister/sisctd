...
namespace DLSisCtd
{
    public class DL_T_Posicion
    {
        ...
        {
            sSql = "select * from T_Posicion where IdPosicion = '" + IdPosicion + "'";
        ...
        {
            sSql = "select count(*) from T_Posicion where IdPosicion ='" + sIdPosicion + "' ";
            return (Convert.ToInt32(ConexionDAO.fEscalar(sSql)) > 0 ? true : false);
        }
        #endregion
        ...
        public void Insertar(BE_T_Posicion oBE_T_Posicion)
        {
            ...
            sSql += "(";
            sSql += "'" + oBE_T_Posicion.IdCliente + "',";
            sSql += "'" + oBE_T_Posicion.IdPosicion + "',";
            sSql += "'" + oBE_T_Posicion.Descripcion + "', ";
            sSql += "'" + (oBE_T_Posicion.Estado ? "1" : "0") + "',";
            sSql += "convert(varchar,getdate(),112),convert(varchar,getdate(),108),'" + oBE_T_Posicion.UsuarioRegistro + "') ";
        ...
        public void Modificar(BE_T_Posicion oBE_T_Posicion)
        {
            ...
            sSql += "where  IdCliente = '" + oBE_T_Posicion.IdCliente + "' and IdPosicion='" + oBE_T_Posicion.IdPosicion + "' ";
            ConexionDAO.fExecute(sSql);
        }
        public void Eliminar(string sIdPosicion)
        {
            sSql = "delete from T_Posicion where IdPosicion = '" + sIdPosicion + "'  ";
            ConexionDAO.fExecute(sSql);
        }
